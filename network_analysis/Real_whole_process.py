...
# 파일 경로 설정
network_dir = "repeat_num/Dongreal_result/Dongreal/real_attack_data"  # 네트워크 파일들이 있는 폴더 경로
output_graph_dir = "network_analysis/Real"  # 평균 그래프를 저장할 폴더
os.makedirs(output_graph_dir, exist_ok=True)

# 네트워크 파일 리스트
network_files = [
    f"{network_dir}/real_network_{i}_average_shortest_paths.csv" for i in range(1, 51)
]
...
derivative_avg = diff_avg_shortest_path

# y축 범위 설정: 미분값의 최대 절댓값 기준
max_derivative = np.max(np.abs(derivative_avg))
ylim_derivative = 1.2 * max_derivative
...
plt.ylim(-ylim_derivative, ylim_derivative)
plt.xlim(0, 2426)
plt.grid(True)
plt.legend()
...
ylim_second_derivative = 1.2 * max_second_derivative
...
print("Real network of all graphs are saved.")
