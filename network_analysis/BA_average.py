...
output_avg_graph_dir = "network_analysis/BA/Average_graph"  # 평균 그래프를 저장할 폴더

# 폴더 생성 (존재하지 않으면)
os.makedirs(output_avg_graph_dir, exist_ok=True)

# 네트워크 파일 리스트 (BA 네트워크 파일 경로)
ba_network_files = [f"{network_dir}/BA_network_{i}_paths.csv" for i in range(1, 11)]
...
plt.title("Average Shortest Path vs Edge Attack Number (BA Networks)")
plt.grid(True)
plt.legend()
